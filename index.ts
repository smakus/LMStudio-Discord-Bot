...
import { LMStudioClient, LLMDynamicHandle, Chat } from '@lmstudio/sdk';
...
const LMTOKEN = process.env.LM_API_TOKEN!;

console.log('Token:', process.env.LM_API_TOKEN);

let modelIdentifier: string | null = null;

async function getLLMSpecificModel(): Promise<LLMDynamicHandle> {
  // create the client
...
async function getModelResponse(userMessage: string, model: LLMDynamicHandle) {
    const now = new Date().toLocaleString();

    const chat = Chat.from([
    { role: 'system', content: 'You are a helpful discord bot responding with short and useful answers. Your name is Joshua. The current date and time is ' + now },
    { role: 'user', content: userMessage },
  ]);

  let response = '';
  for await (const { content } of model.respond(chat)) {
    response += content;
  }
  return response;
}
...
async function getModelResponse(userMessage: string, model: LLMDynamicHandle) {
  ...
  if (!previousResponseId) {
    const now = new Date().toLocaleString();
    body.system_prompt = 'You are a helpful discord bot responding with short and useful answers. Your name is Joshua. The current date and time is ' + now;
  } else {
  ...
  return data.output[0].content as string;
}
